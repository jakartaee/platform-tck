...
The Jakarta Platform, Enterprise Edition CI, Eclipse GlassFish 5.1 provides the container-managed
...
[[GEXPI]][[jakarta-registries-ts.jte-properties]]

5.4.27.2 Jakarta Registries ts.jte Properties
+++++++++++++++++++++++++++++++++++++++++++++

For Jakarta Registries testing, make sure the following properties have been set in
the `<TS_HOME>/bin/ts.jte` file:

* `authenticationMethod`: Authentication method for the Jakarta Registries provider;
...
RegistryServer, the UDDI Registry that comes with the _Java Web Services
Developers Pack_: `registryURL = http://localhost:8080/RegistryServer/`
...
username/password or digital certificates for Jakarta Registries credentials:
