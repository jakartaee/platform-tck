...
This chapter describes how to set up the Jakarta EE 10 Platform TCK test suite and
configure it to work with your test environment. It is recommended that
you first set up the testing environment using the Jakarta EE 10 CI and then
with your Jakarta EE 10 server.

This chapter includes the following topics:

* link:#GEWTL[Allowed Modifications]
* link:#GEWTP[Configuring the Test Environment]
* link:#GEWTQ[Configuring a Jakarta EE 10 Server]
...
Jakarta EE 10 CI and its associated database in order to pass the TCK. A
...
1.  Identify the software pieces and assemble them into the Jakarta EE 10
platform to be tested for certification.
2.  Implement the porting package APIs. +
Some functionality in the Jakarta EE 10 platform is not completely specified
by an API. To handle this situation, the Jakarta EE 10 Platform TCK test suite
...
particular Jakarta EE 10 server environment. See
...
See link:#GEWUI[Section 5.3.2, "Configuring the Jakarta EE 10 CI as the
VI,"] for a list of the modifications that must be made to run CTS
against the Jakarta EE 10 CI.
4.  Set up the vendor's Jakarta EE 10 server implementation (VI). +
...
against the vendor's Jakarta EE 10 server.
...
[[GEWTQ]][[configuring-a-jakarta-ee-10-server]]

5.3 Configuring a Jakarta EE 10 Server
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This section describes how to configure the Jakarta EE 10 server under test.
You can run the TCK tests against the Jakarta EE 10 CI or your own Jakarta
...
EE 10 CI servers, one of which must be a Jakarta EE 10 CI using, or configured to
...
* Compatible Implementation (CI): Jakarta EE 10 CI, for example, Eclipse GlassFish 6.1
* Vendor Implemention (VI): Jakarta EE 10 implementation from a vendor other
than Eclipse; typically, the goal of running the TCK is to certify a Jakarta
EE 10 VI; in some cases, for purposes of familiarizing yourself with TCK,
you may choose to run the Jakarta EE 10 CI as the VI
* Bundled Derby: Apache Derby database bundled with the Jakarta EE 10 CI, Eclipse GlassFish 6.1
...
There are three general scenarios for configuring Jakarta EE 10 servers for
Jakarta EE 10 Platform TCK testing (Note: in the following images, Java EE refers to Jakarta EE. 
RI should be replaced with CI for Compatible Implementation):

* Configure the Jakarta EE 10 CI as the server under test +
image:img/serverpath-ri.png["CI as Server Under Test"]

Use the Jakarta EE 10 CI as the Jakarta EE 10 VI; you may want to do this as a
sanity check to make sure you are comfortable with using the Jakarta EE 10
TCK against a known standard CI with certified sample applications
before proceeding with tests against your Jakarta EE 10 VI. See
link:#GEWUI[Section 5.3.2, "Configuring the Jakarta EE 10 CI as the VI,"]
for instructions.

* Configure your Jakarta EE 10 VI as Server Under Test +
image:img/serverpath-vi.png["VI as Server Under Test"]

This is the primary goal of using the Jakarta EE 10 Platform TCK; you will eventually
need to configure the Jakarta EE 10 implementation you want to certify. See
link:#GEWWA[Section 5.3.3, "Configuring Your Application Server as the
VI,"] for instructions.

* Configure two Jakarta EE 10 servers for the purpose of interop testing +
image:img/serverpath-interop.png["Two Servers for Interop Testing"]

Rebuildable tests require that you configure two Jakarta
EE 10 servers on one or two machines. One server will be your Jakarta EE 10
VI running a database of your choice with JDBC 4.1-compliant drivers.
The second server must be the Jakarta EE 10 CI using the bundled Java DB
database. 

In terms of the Jakarta EE 10 Platform TCK, all TCK configuration settings are made
in the `<TS_HOME>/bin/ts.jte` file. When configuring a Jakarta EE 10 server,
...
5.3.2 Configuring the Jakarta EE 10 CI as the VI
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To configure the Jakarta EE 10 CI as the server under test (that is, to use
the Jakarta EE 10 CI as the VI) follow the steps listed below. In this
scenario, the goal is simply to test the Jakarta EE 10 CI against the CTS
...
EE 10 TCK for similar instructions.
...
If you are using the Jakarta EE 10 CI, execute the following command from
the command line: +
+
[source,oac_no_warn]
----
...
To use a Jakarta EE 10 server other than the Jakarta EE 10 CI, follow the steps
below.
...
using Ant configuration target similar to the target for the Jakarta EE 10
...
.  Start your Jakarta EE 10 server.

.  [[BABDADHA]] Set up required users and passwords.

..  Set up database users and passwords that are used for JDBC
connections. +
The Jakarta EE 10 Platform TCK requires several user names, passwords, and
...
against the Eclipse GlassFish 6.1 Jakarta EE 10 CI application server. It is required in this case
because the management Jakarta Enterprise Bean (MEjb) in the Jakarta EE 10 server is protected
...
. Configure your Jakarta EE 10 server to use the appropriate logical name
...
. [[BABEGCJH]] Install the client-side certificate in the `trustStore` on the Jakarta EE 10
server.  +
...
policy file or files on your Jakarta EE 10 server. +
...
Jakarta EE 10 server and the server that is running the Jakarta EE 10 CI. +
...
.. Restart your Jakarta EE 10 server.
...
* Set the `javaee.home.ri` property to the location where the Jakarta EE 10
CI is installed.

* Set the `wsgen.ant.classname` property to the Jakarta EE 10 application
...
----
+
* Set the `wsimport.ant.classname` property to the Jakarta EE 10 application
...
Before you can run any of the technology-specific Jakarta EE 10 Platform TCK tests,
...
When configuring the Jakarta EE 10 Platform TCK for the Windows environment, set the
...
used to identify the name of a logical host that processes Jakarta EE 10
requests. Jakarta EE 10 requests may be directed to a logical host using
...
Jakarta EE 10 profile's `AppContextId` hostname. This property is used by
the Jakarta EE 10 security tests as well as by the
...
Jakarta EE 10 CI as the VI,"] for more information about setting these
properties.
...
Jakarta RESTful Web Services tests using the Jakarta EE 10 Compatible Implementation and/or a
...
5.4.8.1 To Configure Your Environment to Run the Jakarta RESTful Web Services Tests Against the Jakarta EE 10 CI
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
5.4.8.2 To Package WAR files for Deployment on the Jakarta EE 10 CI
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

The Jakarta EE 10 Platform TCK test suite does not come with prebuilt test WAR files
for deployment on Jakarta EE 10 CI. The test suite includes a command to
generate the test WAR files that will be deployed on the Jakarta EE 10 CI.
...
To package the Jakarta RESTful Web Services WAR files for deployment on the Jakarta EE 10 CI,
...
vendor's Jakarta EE 10-compliant application server.
...
A vendor must create VI-specific Jakarta EE 10-compliant WAR files so the
VI-specific Servlet class will be included instead of the Jakarta EE 10
CI-specific Servlet class.

All resource and application class files are already compiled. The
Vendor needs to package these files. Jakarta EE 10 Platform TCK makes this task
easier by including template WAR files that contain all of the necessary
files except for the VI-specific servlet adaptor class. The Jakarta EE 10
...
descriptor also comes with the Jakarta EE 10 CI, Eclipse GlassFish 6.1, and has the values for the
...
The Jakarta EE 10 Platform TCK test suite provides a tool,
`${ts.home}/bin/xml/impl/glassfish/jersey.xml`, for the Jakarta EE 10 CI
...
The Jakarta Connector tests verify that a Jakarta EE 10 server correctly implements
the Jakarta Connector V1.7 specification. The Connector compatibility tests
ensure that your Jakarta EE 10 server still supports the Connector V1.0
functionality.

The `config.vi` target is run to configure the Jakarta EE 10 server for
...
The RAR files that are used with Jakarta EE 10 Platform TCK test suite differ from
those that were used in earlier test suites. Jakarta EE 10 Platform TCK no longer
...
See section EE.8.2.2 of the Jakarta EE 10 platform specification and section
20.2.0.1 in the Jakarta Connectors (formerly JCA) 1.7 specification, which explicitly state that the
resource adapter server may employ the library mechanisms in Jakarta EE 10.
...
Jakarta EE 10 Platform TCK has scenarios in which multiple standalone RAR files that
...
in the Jakarta EE 10 platform specification. If both RAR files (RAR1 and
...
The Jakarta EE 10 Platform TCK test suite treats the `whitebox.jar` dependency as an
...
See section EE.8.2.2 in the Jakarta EE 10 platform specification for
...
Jakarta EE 10 CI, Eclipse GlassFish 6.1 to create this connection pool: 
+
[source,oac_no_warn]
----
...
For example, you could use the `asadmin` command line utility in the
Jakarta EE 10 CI to create the three connection pool elements: 
+
[source,oac_no_warn]
----
...
element, they use the same pool connection at runtime. Jakarta EE 10 Platform TCK
does reuse the same connection pool ID for testing the Jakarta EE 10 CI Eclipse GlassFish 6.1.
...
the `ts.jte` file. +
This step must be completed for Jakarta EE 10 and Jakarta EE 10 Web Profile
testing.
...
Jakarta EE 10 Platform TCK does not have a property that you can set to configure the
...
an XA datasource in the Jakarta EE 10 timer implementation. For example:
...
you may be using, as explained in link:#GEWUI[Configuring the Jakarta EE 10
CI as the VI].
...
special setup to run. This is no longer the case, since Jakarta EE 10 Platform TCK
...
`src/com/sun/ts/tests/ejb30/persistence` directory. The Jakarta EE 10 tests are now in
...
This section explains how to set up and configure the Jakarta EE 10 Platform TCK test
...
name of the Jakarta Messaging Resource Adapter in the Jakarta EE 10 CI.
...
required profile for Jakarta EE 10 Platform TCK. There are other optional profile
...
`<JAVAEE_HOME>` is the location of your Jakarta EE 10 CI installation.
...
To comply with Jakarta EE 10 requirements, Jakarta Authorization must be supported in both
...
When running Jakarta Authorization tests against the Jakarta EE 10 CI, if you need to restart
...
5.4.18 WSDL: Webservice Test and Runtime Notes
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
5.4.18.1 WSDL ts.jte Properties
+++++++++++++++++++++++++++++++
...
5.4.18.2 Webservice Endpoint WSDL Elements
++++++++++++++++++++++++++++++++++++++++++
...
5.4.18.2.1 Setting Endpoint Address
...
5.4.18.2.2 Jakarta Enterprise Beans Endpoint Security
...
5.4.18.2.3 Transport Guarantee
...
5.4.18.2.4 Publishing Final WSDL During Deployment
...
5.4.18.3 Webservice Client WSDL Elements
++++++++++++++++++++++++++++++++++++++++
...
5.4.18.3.1 Resolving Container-Managed Ports
...
5.4.19 Jakarta Security API Test Setup
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
5.4.20.1 sigTestClasspath Property
++++++++++++++++++++++++++++++++++
...
5.4.20.2 Additional Signature Test Information
++++++++++++++++++++++++++++++++++++++++++++++

The Jakarta EE 10 Platform TCK signature tests perform verifications in two different
...
implementation needs a modification to exactly match the Jakarta EE 10 Platform SPEC API.
...
5.4.21.1 Setup Considerations for MySQL
+++++++++++++++++++++++++++++++++++++++
...
5.4.21.2 Setup Considerations for MS SQL Server
+++++++++++++++++++++++++++++++++++++++++++++++
...
To configure the JavaTest harness to run the Jakarta EE 10 Platform TCK tests,
