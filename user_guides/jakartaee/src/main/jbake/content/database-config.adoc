...
For the purposes of Jakarta EE 9.1 Platform TCK testing, all database configuration
...
link:config.html#GEWTQ[Configuring a Jakarta EE 9.1 Server]. The final step,
...
tested against the Jakarta EE 9.1 Platform TCK, the database must be initialized for
...
are read by the Jakarta EE 9.1 Platform TCK when you start the test suite. The DDL and
DML files are described later in this appendix, in link:#GFKOC[Database
DDL and DML Files].

The Jakarta EE 9.1 Platform TCK includes the following database-specific Ant targets:
...
For each supported database type, the Jakarta EE 9.1 Platform TCK includes a set of
...
In addition to the database-specific DML files, the Jakarta EE 9.1 Platform TCK
