...
A new logging mechanism was introduced with Jakarta EE 8 CTS. Logging now
occurs as follows:
...
When the server that is running the Jakarta EE 8 CI is started, a set of
...
If you can not access the page at URL on your Jakarta EE 8 CI, all CSIv2
...
forward means from licensee server to the Jakarta EE 8 CI server, and
reverse means from the Jakarta EE 8 CI server to the licensee server. In
