...
You run the `ant enable.jaspic` command to configure the Jakarta EE 11 CI to
run the Jakarta Authentication tests. After running the Jakarta Authentication tests, you run the
`ant disable.jaspic` command to restore the Jakarta EE 11 CI to the state it
was in before you configured it for running the Jakarta Authentication tests. This is
required because Jakarta Authentication replaces some of the Jakarta EE 11 CI's default
...
Jakarta EE 11 Platform TCK uses a Test Suite SPI Verifier (TSSV) to verify whether
...
For the Jakarta EE 11 Platform TCK, we register more than one provider in vendor's
