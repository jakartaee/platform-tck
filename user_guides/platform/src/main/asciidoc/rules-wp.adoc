...
EE-WP20 Compatibility testing for the Jakarta EE {tck_version} Web Profile consists ofrunning the tests for the technologies defined in
<<jakarta-ee-technologies-tested-with-jakarta-ee-platform-tck>> If optional technologies defined in the Jakarta EE {tck_version} Web Profile platform are implemented in addition to the required Jakarta EE {tck_version} Web Profile technologies, corresponding tests within this TCK for those additional technologies must also be run.
