...
* link:#GEWTQ[Configuring a Jakarta EE {tck_version} Server]
...
Jakarta EE {tck_version} CI and its associated database in order to pass the TCK. A
...
See link:#GEWUI[Section 5.3.2, "Configuring the Jakarta EE {tck_version} CI as the
VI,"] for a list of the modifications that must be made to run CTS
against the Jakarta EE {tck_version} CI.
...
against the vendor's Jakarta EE {tck_version} server.
...
5.3 Configuring a Jakarta EE {tck_version} Server
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
EE {tck_version} CI servers, one of which must be a Jakarta EE {tck_version} CI using, or configured to
...
* Compatible Implementation (CI): Jakarta EE {tck_version} CI, for example, Eclipse GlassFish 6.1
* Vendor Implemention (VI): Jakarta EE {tck_version} implementation from a vendor other
...
* Bundled Derby: Apache Derby database bundled with the Jakarta EE {tck_version} CI, Eclipse GlassFish 6.1
...
link:#GEWUI[Section 5.3.2, "Configuring the Jakarta EE {tck_version} CI as the VI,"]
for instructions.
...
5.3.2 Configuring the Jakarta EE {tck_version} CI as the VI
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
If you are using the Jakarta EE {tck_version} CI, execute the following command from
the command line: +
+
[source,oac_no_warn]
----
...
using Ant configuration target similar to the target for the Jakarta EE {tck_version}
...
against the Eclipse GlassFish 6.1 Jakarta EE {tck_version} CI application server. It is required in this case
because the management Jakarta Enterprise Bean (MEjb) in the Jakarta EE {tck_version} server is protected
...
* Set the `wsgen.ant.classname` property to the Jakarta EE {tck_version} application
...
----
+
* Set the `wsimport.ant.classname` property to the Jakarta EE {tck_version} application
...
Jakarta EE {tck_version} CI as the VI,"] for more information about setting these
properties.
...
Jakarta RESTful Web Services tests using the Jakarta EE {tck_version} Compatible Implementation and/or a
...
5.4.8.1 To Configure Your Environment to Run the Jakarta RESTful Web Services Tests Against the Jakarta EE {tck_version} CI
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
5.4.8.2 To Package WAR files for Deployment on the Jakarta EE {tck_version} CI
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
descriptor also comes with the Jakarta EE {tck_version} CI, Eclipse GlassFish 6.1, and has the values for the
...
The Jakarta Connector tests verify that a Jakarta EE {tck_version} server correctly implements
...
Jakarta EE {tck_version} Platform TCK has scenarios in which multiple standalone RAR files that
...
does reuse the same connection pool ID for testing the Jakarta EE {tck_version} CI Eclipse GlassFish 6.1.
...
you may be using, as explained in link:#GEWUI[Configuring the Jakarta EE {tck_version}
CI as the VI].
...
This section explains how to set up and configure the Jakarta EE {tck_version} Platform TCK test
...
To configure the JavaTest harness to run the Jakarta EE {tck_version} Platform TCK tests,
