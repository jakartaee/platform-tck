...
* link:#GFADX[Jakarta EE {tck_version} Platform TCK Operating Assumptions]
...
7.1 Jakarta EE {tck_version} Platform TCK Operating Assumptions
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Jakarta EE {tck_version} Platform TCK is installed.
2.  Change to the `<TS_HOME>/bin` directory.
3.  Copy <TS_HOME>/bin/ts.jte.jdk11 as <TS_HOME>/bin/ts.jte for JakartaEE {tck_version}.
...
===== Example 7-1 Running the Jakarta EE {tck_version} Platform TCK Signature Tests
...
For example, you could run all Jakarta EE {tck_version} tests with a status of failed
...
required by the Jakarta EE {tck_version} platform specification. link:#BAGGCEJC[Table
...
against the Jakarta EE {tck_version} CI, you must start the standalone deployment
