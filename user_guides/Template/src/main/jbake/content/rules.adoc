...
* In this section:
* Replaced Maintenance Lead with "Jakarta EE Specification Committee." Must be confirmed 
* Maintenance Lead seems to refer to the organization that "owns" the Spec. I assigned this to the Specification Committee. The definition refers to the Specification Process as well as the specification project team.
* Java Partner Engineering is replaced with Eclipse Jakarta EE Specification Project 
website (see {techID}2. Must be confirmed
...
Jakarta EE Specification Committee, excluding those tests on the 
...
Jakarta EE Specification Committee, that are not required to be passed to certify
...
|Libraries a|
The class libraries, as specified through the Eclipse Foundation, Specification 
Process (EFSP), as refined by the Jakarta EE Specification Process (JESP), for the Technology Under Test.
...
|Maintenance Lead |The Jakarta EE Specification Committee is responsible for
maintaining the Specification, and TCK for the Technology. 
The Specification Project Team will propose revisions and updates to the Jakarta EE 
Specification Committee which will approve and release new versions of the 
specification and TCK. 
Eclipse Jakarta EE Specification Committee is Maintenance Lead for 
{TechnologyFullName}.
...
against a known Jakarta EE 8 CI verifies that the mechanism generates
...
|Specifications a|
The documents produced through the Eclipse Foundation, Specification 
Process (EFSP), as refined by the Jakarta EE Specification Process (JESP)
...
through the Eclipse Foundation, Specification 
Process (EFSP), as refined by the Jakarta EE Specification Process (JESP).
...
|Version |A release of the Technology, as produced through the Eclipse Foundation, Specification 
Process (EFSP), as refined by the Jakarta EE Specification Process (JESP).
|=======================================================================
...
Conformance Tests would be posted to the Eclipse Jakarta EE Specification Project 
...
project web-site. This site can be found from the Jakarta EE, EE4J Project page,
then selecting the sub-project for {TechnologyFullName}.
