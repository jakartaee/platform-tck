...
{TechnologyRIURL} for more information. The {TechnologyShortName} {TechnologyVersion}
APIs have also been integrated into the Java EE 7 RI bundle, which is available
from Java Licensee Engineering.

See the Concurrency TCK Release Notes for more specific information about Java SE
version requirements, supported platforms, restrictions, and so on.
...
details about the following steps.
  ...
  c.  Configure and start the Web container.
  d.  Configure the JavaTest harness.
  e.  Deploy the {TechnologyShortName} TCK tests.
